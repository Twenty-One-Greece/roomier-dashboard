...
class Store_Bookings {
  ...
  getBookingDates(roomTypeID, rateID, propertyID) {
    ...
    const URL         = config.dashboardAPI + '/user/' + userID + '/properties/' + propertyID + "/roomTypes/" + roomTypeID + "/bookings/"
  ...
  getBookingsFull(propertyID) {
    const userID      = window.localStorage.id;
    const token       = window.localStorage.token;
    const URL         = config.dashboardAPI + '/user/' + userID + '/properties/' + propertyID + "/bookingsFull/"
    const headers     = {headers: config.headers(token)}
  ...
  deleteBooking(bookingID, propertyID) {
    const userID      = window.localStorage.id;
    const token       = window.localStorage.token;
    const URL         = config.dashboardAPI + '/user/' + userID + '/properties/' + propertyID + "/bookings/" + bookingID
    const headers     = {headers: config.headers(token)}
